...
## 2026-09-15 내부 의존성 반영

각 저장소의 구현은 [내부 의존성 점검](internal-dependencies.md)을 기준으로
기존 이슈에 반영했다. 관찰된 모든 차이를 새로운 선행 이슈로 만들지 않는다.

- 첫 착수는 **#12 기존 Deployment 연결·실행**이다. **#11 선택적 topology 읽기**도
  독립적으로 ready다. 새 graph template, DeepAgents ADR, beta.4 배포는 시작 조건이 아니다.
- **#28**은 기존 계측 이슈를 확장해 실제 graph producer 연결과 pause Step 경계를
  소유하며 Slice 1에 둔다. outcome-only observer에서 duration을 추측하지 않고 공개
  lifecycle seam을 확인해야 한다. 이 확인은 #12의 API 연결을 막지 않는다.
- **#14**가 Aegra의 새 API Run ID를 같은 논리 Run에 연결하는 지속성 계약을 소유한다.
  #28의 작은 계측 fixture는 논리 identity를 명시적으로 주입해 이 의존을 순환시키지 않는다.
- #16은 #12 이후, #17은 #16·#14 이후, #18·#19는 #17 이후, #20은 #13·#14 이후다.
  마일스톤 순서를 기능 간 불필요한 전체 직렬 의존으로 만들지 않는다.
- #19의 최초 입력은 겹쳐 실행하는 서로 다른 Subject 둘과 같은 Subject 충돌 쌍이다.
  20개 실행은 독립적인 측정 이유가 생길 때만 추가한다.
- #1과 Slice 0·0.5의 추적 상태는 이미 기록된 검증 증거에 맞춘다. 이번 정리는
  새 통합 검증 통과나 내부 업무 그래프의 운영 준비 완료를 뜻하지 않는다.

실행 시점의 본문·준비 라벨·native 선행 관계는 GitHub 이슈를 다시 확인한다.
