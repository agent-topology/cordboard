...
## 추가 — 인터럽트와 재개의 span 경계 (2026-09-12)

`awaiting_approval`은 처음부터 Step Outcome 목록에 있었지만, **그 Step의 span이
승인 대기 동안 열려 있는지**는 어디에서도 정하지 않았다. 정하지 않고 두면 구현이
"열어 둔다"를 고른다. 그게 자연스러운 쓰기 순서이기 때문이다.

**규칙.** `interrupt()`가 발생하면 Step span을 그 자리에서 닫는다. Outcome은
`awaiting_approval`이고 span status는 오류가 아니다. 열려 있는 Attempt가 있으면
Step보다 먼저 닫는다. 재개는 **새 Step span**을 열고 `cord.resumed_from`에 직전
Step의 span_id를 적는다. 두 span 사이의 공백은 사람의 판단 시간이며 Run에 속하고
Node에 속하지 않는다.

**근거는 설계가 아니라 실측이다.** 같은 저자의 `workbench`(ghpr)가 이 규칙을
ADR-006에 갖고 있고, 이유를 한 달치 데이터에서 얻었다 — 밤새 멈춘 Run이 14시간짜리
노드 duration을 만들어 그 뒤의 모든 지연 통계를 오염시킨다. 그 프로젝트의 표현대로
"한 달치 데이터가 쌓이기 전에는 보이지 않는" 결함이다. 우리는 아직 그 데이터가 없고,
그래서 스스로는 발견할 수 없었다.

**중첩 모델에서 피해가 더 크다.** `workbench`는 노드 span이 평평해서 오염이 그 노드
하나에서 끝난다. 여기서는 Step이 Attempt들을 담기 때문에, 열어 둔 Step은 자기
duration뿐 아니라 "Step 시간 대비 Attempt 시간" 같은 파생 지표까지 함께 망가뜨린다.
층을 하나 더 갖는 대가로, 이 규칙은 선택이 아니라 필수가 된다.

**아카이브 계약에 미치는 영향.** 한 Node 실행이 여러 Step span으로 나뉠 수 있게
된다. `archive-escalations`는 승격된 Attempt를 세므로 영향받지 않는다. Step 단위로
세는 질의를 나중에 쓸 때는 `cord.resumed_from` 사슬을 하나의 논리 Step으로 접어야
한다. [ADR-0003](0003-subject-not-thread.md)이 논리 Run의 재개 연속성을 후속으로
미뤄 둔 것과 같은 자리의 문제이며, 같은 사슬 규약을 쓴다.

이 규칙은 슬라이스 2(#14·#15)가 실제 데이터에서 처음 시험한다. 그전까지는 선언이다.

**교환의 반대쪽.** `workbench`는 이 결정과 같은 날 Step 층을 도입했다. 그쪽 ADR-006의
형제 규칙("attempt 2를 attempt 1 안에 넣지 마라")은 우리와 충돌한 적이 없다 — 우리도
