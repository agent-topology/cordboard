...
def route_signal(board_dir: Path, signal: dict, *, timeout: float = 120.0) -> dict:
    ...
    payload content), or ``{"status": "execution_failed", ..., "error": ...}``
    when a matched, well-mapped Rule's target could not be reached or did not
    accept the Run -- ``error`` is ``execute()``'s own payload-free message.
    """
    ...
    try:
        result = execute(connection["endpoint"], rule["assistant"], subject, graph_input, timeout=timeout)
    except (ValueError, RuntimeError) as exc:
        # aegra_client.execute's exceptions are already payload/credential-free.
        return {"status": "execution_failed", "signal_id": signal["id"], "rule": rule["name"], "error": str(exc)}
